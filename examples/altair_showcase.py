"""Altair theme showcase — adapted from https://vega.github.io/vega-themes/

Run with:
    python examples/altair_showcase.py
"""
...
# ── activate the Charité theme ─────────────────────────────────────────────
enable(palette="primary")
# Override palette or font with e.g.:
#   enable(palette="goldelse")
#   enable(font="Arial", font_size=13)
...
iowa_elec    = data.iowa_electricity.url
...
area = (
    alt.Chart(alt.UrlData(iowa_elec), height=STANDARD, width=STANDARD, title="Area")
...
compound_chart = (
    ...
    title=alt.Title(
        "Charité Vega-Altair Theme Test",
        fontSize=20,
...
# ── save / display ─────────────────────────────────────────────────────────
out = pathlib.Path(__file__).parent / "charite_altair_showcase.html"
compound_chart.save(str(out))
print(f"Saved → {out}")

# In a notebook, just use: compound_chart
compound_chart